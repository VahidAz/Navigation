...
void SkidSteerDrivePlugin::OnVelMsg(ConstPosePtr &_msg)
{
  ...
  double right_vel = _msg->position().x();
  double left_vel  = _msg->position().y();

  this->Joints[RIGHT_FRONT]->SetVelocity(0,right_vel); // vel_lin - vel_rot);
  this->Joints[RIGHT_REAR ]->SetVelocity(0,right_vel); // vel_lin - vel_rot);
  this->Joints[LEFT_FRONT ]->SetVelocity(0,left_vel ); // vel_lin + vel_rot);
  this->Joints[LEFT_REAR  ]->SetVelocity(0,left_vel ); // vel_lin + vel_rot);
}
